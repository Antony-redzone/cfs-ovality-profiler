...
void XYDiameter::FillHole(int right, int left, vec2double rightHeight, vec2double leftHeight)
{
	...
	botPointsRight = botPoints[right];
	botPointsLeft = botPoints[left];

	if(botPointsRight > 179) return;
	if(botPointsRight < 0) return;
	if(botPointsLeft > 179) return;
	if(botPointsLeft < 0) return;
