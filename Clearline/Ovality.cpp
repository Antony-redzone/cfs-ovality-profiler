...
void Ovality::CalculateFrameOvality(void)
{
	...
	//ExportPoints(currentFrame, calculatedOvality);
}


void Ovality::ExportPoints(int currentFrameNo, float ovality)
{
	FILE *f;
	float x;
	float y;
	...
	f = fopen("c:\\test\\ovalityExport.csv","a+");

	sprintf(writeLine,"%i,%f",currentFrameNo,ovality);
	for(i=0;i<180;i++)
	{
		sprintf(writeLine,"%s,%f",writeLine,fakePoints[i].x);
	}
	...
	for(i=0;i<180;i++)
	{
		sprintf(writeLine,"%s,%f",writeLine,fakePoints[i].y);
	}
	...
	for(i=0;i<180;i++)
	{
		sprintf(writeLine,"%s,%f",writeLine,filteredPoints[i]);
	}
