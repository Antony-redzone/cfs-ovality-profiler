...
double VERSION =		5.0; // Fixed max and min, some boundry isues, initialise of max min not done
						//4.9; // Fixed -1000000000 bug in smooth grah calculation
						//4.8; //PCN6314 bung end frame inclination calc
						//4.7; //Inclination adjustment
						//4.6; //inclination lay of pipe calculation v Richard
...
void __stdcall clearLine_SmoothOutGraphSingle(float *_dataGraphSource, float *_dataGraphDest, int _numberFrames, int abs) //0 for not absolute, 1 for absolute (0 for capacity)
{
	...
	for(i=0;i<_numberFrames;i++)
	{
		...
		if(abs != 0)
		{
			for(j=0;j<5;j++) 
			{
				if(_dataGraphSource[i+j-2+adjust] ==-1000000000) sampleData[j]=-1000000000;
				else sampleData[j]=fabs(_dataGraphSource[i+j-2+adjust]);
			}
		}
...
void __stdcall clearLine_SmoothOutGraphDouble(double *_dataGraphSource, float *_dataGraphDest, int _numberFrames, int abs) //0 for not absolute, 1 for absolute (0 for capacity)
{
	...
	for(i=0;i<_numberFrames;i++)
	{
		...
		if(abs != 0)
		{
			...
			{
				if (_dataGraphSource[i+j-2+adjust]==-1000000000) sampleData[j]=-1000000000;
				else sampleData[j]=(float) fabs(_dataGraphSource[i+j-2+adjust]);
			}
		}
...
void __stdcall clearline_calculateLayOfPipe_Richard(double startHeight, double endHeight,
													...
													int fromFrame, int toFrame,
													int totalFrames,
													double count, double expectedDiameter)
{
	double distanceBetweenFrames;
	double distanceBetweenFramesUnscaled;
	...
	adjustedFrameCount = 10;
	if((distance[toFrame-1] - distance[1]) == 0) return; //PCNcrashing 623??? '12 September 2011

	fallBetweenRL = (endHeight - startHeight) / (distance[toFrame-1] - distance[1]);


	midFrame = (toFrame - 1 / 2);
	...
	{
		gradientSum = 0;
		for (i=adjustedFrameCount-4;i<=adjustedFrameCount+4;i++)
		{
			if ((frameNo+i)<=toFrame)
			{
				distanceBetweenFrames = fabs(distance[frameNo+i] - distance[frameNo])*scale;
				heightBetweenFrames = centre[frameNo+i] - centre[frameNo];
			}
			else
			{
				distanceBetweenFrames = fabs(distance[frameNo-i] - distance[frameNo])*scale;
				heightBetweenFrames = (centre[frameNo] - centre[frameNo-i]);// * -1;
			}
		...
		gradient = gradientSum / 9;
		
		if ((frameNo+adjustedFrameCount+4)<=toFrame)
		{
			distanceBetweenFramesUnscaled = fabs(distance[frameNo+adjustedFrameCount] - distance[frameNo]);
			//currentGradient = currentGradient - gradient;
		}else{
			distanceBetweenFramesUnscaled = distanceBetweenFramesUnscaled;
			//currentGradient = currentGradient;
		}
		
		currentGradient = currentGradient - gradient;

		height = (currentGradient * distanceBetweenFramesUnscaled);

		adjustedHeight = startHeight - ((fallBetweenRL) * ((distance[1])-(distance[frameNo]))) - height;
