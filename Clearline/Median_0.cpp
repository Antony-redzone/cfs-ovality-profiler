...
int Median::CreateFakePoints(long index)  //PCN4314
{
	long point;
	long pointIndex;
	long pointLoop;
	...
	for(point=0;point<180;point++)
	{
		...
		if(egnoreList[point]==1) {numberOfZero++; continue;}
		if((pvDataX[pointIndex]==0) && (pvDataY[pointIndex]==0)) {numberOfZero++; continue;}
		firstGoodPoint=pointIndex;
		break;
	}
	if(firstGoodPoint==-1)  return -1; // No good points
	if(numberOfZero>90) return -1; // Not enough good points
...
double Median::GetRightSideHeight(long index, double leftHandHeight, long &pointLoop)  //PCN4314
{
