...
Public Const FishEyeTFactorMax As Integer = 300
...
Dim tempPVDConfigVersion As Single
tempPVDConfigVersion = GetPVDVer 'CDbl(Right(ConfigInfo.PVDFileVersion, Len(ConfigInfo.PVDFileVersion) - 1))

    If ConfigInfo.FishEyeDistortion = 0 Then
        Call FEOFF ': Exit Function
        If ConfigInfo.FishEyeHorDistortion = 1 Then Exit Function 'PCN4474
    End If
...
ProfilerMessageBox.ProfilerMsgBoxLbl.Caption = DisplayMessage("Can not read this file, loading default settings."): ProfilerMessageBox.Show vbModal: ProfilerMessageBox.ZOrder 0

Call FECLoadDefaultSettings

Exit Function
Err_Handler:
MsgBox Err & "-FF1:" & Error$
End Function
...
Dim CalibrationDistance As String
Dim CalibrationLineLength As String

' set the original
Call getimagesize(currentheight, currentwidth)

Call GetINI_ParameterInfoOnly(MyFile, "Fish_OriginalHeight=", height): ConfigInfo.FishEyeOriginalHeight = SafeCDbl(height)
Call GetINI_ParameterInfoOnly(MyFile, "Fish_OriginalWidth=", width):   ConfigInfo.FishEyeOriginalWidth = SafeCDbl(width)

Call setoriginalsize(CLng(width), CLng(height))

Call GetINI_ParameterInfoOnly(MyFile, "Fish_Ratio=", fe_scale): ConfigInfo.FishEyeRatio = SafeCDbl(fe_scale)
Call GetINI_ParameterInfoOnly(MyFile, "Fish_Distortion=", FEDistortion): ConfigInfo.FishEyeDistortion = SafeCDbl(FEDistortion)
Call GetINI_ParameterInfoOnly(MyFile, "Fish_DistortionHorizontal=", FEDistortionHor): ConfigInfo.FishEyeHorDistortion = SafeCDbl(FEDistortionHor)
Call settfactor(1 + (ConfigInfo.FishEyeDistortion / FishEyeDistortionDivider))

Call GetINI_ParameterInfoOnly(MyFile, "Fish_CenterX=", FE_X_Centre): ConfigInfo.FishEyeCenterX = SafeCDbl(FE_X_Centre)
Call GetINI_ParameterInfoOnly(MyFile, "Fish_CenterY=", FE_Y_Centre): ConfigInfo.FishEyeCenterY = SafeCDbl(FE_Y_Centre)
Call setfecentre(SafeCDbl(FE_X_Centre), SafeCDbl(FE_Y_Centre))

'CPN4453, swaped the two below
Call setscalevalue(SafeCDbl(fe_scale)) ' * height / currentheight)) 'PCN4616
Call hough_SetYFishScale(ConfigInfo.FishEyeHorDistortion)

        
ConfigInfo.Ratio = CalLen_Global / CalLength_Global 'PCN3035 'PCN364

If (ConfigInfo.FishEyeDistortion <> 0) Or (ConfigInfo.FishEyeHorDistortion <> 1) Then
    Call FEON
Else
    Call FEOFF
End If
...
Call GetINI_ParameterInfoOnly(MyFile, "CalibrationDistance=", CalibrationDistance)
Call GetINI_ParameterInfoOnly(MyFile, "CalibrationLineLength=", CalibrationLineLength)

CalLen_Global = SafeCDbl(CalibrationDistance) 'PCN4616
CalLength_Global = SafeCDbl(CalibrationLineLength) 'PCN4616
...
OptionsPage.FishEyeCameraDropdown.text = FecCameraModel 'PCN3595 if loaded show camera, (25 Oct 2005, Antony)

Exit Function
Err_Handler:
MsgBox Err & "-FF2:" & Error$
End Function
...
'                                                                  'distortion in 0 then it is displayed as off
    ...
    'PCN4474 Only if no fisheye distorition AND no VERTICAL!!!! distortion do you leave the fish eye distortion off.
    If ConfigInfo.FishEyeHorDistortion = 0 Then ConfigInfo.FishEyeHorDistortion = 1
    If ConfigInfo.FishEyeDistortion = 0 And ConfigInfo.FishEyeHorDistortion = 1 Then 'CPN3595
        Call FEOFF
    Else
        Call turnfisheyeon
...
Err_Handler:
    Select Case Err
        ...
        Case Else: MsgBox Err & "-FF9:" & Error$
    End Select
End Function
...
For Y = 1 To Config_LineCnt
  ...
  If SectionHead = "[FishEyeParameters]" Then
     ...
     Do Until SectionDetail = "" Or Left(SectionDetail, 1) = "[" Or SectionDetail = "\\" 'End marker of data
      If SectionDetail <> "" Then Call GetParam(SectionDetail, Parameter, value, PathName, FileName)
      Select Case Parameter
        Case "Fish_Distortion=":            FEDistortion = SafeCDbl(value)
        Case "Fish_CenterX=":               FECentreX = SafeCDbl(value)
        Case "Fish_CenterY=":               FECentreY = SafeCDbl(value)
        Case "Fish_OriginalWidth="
            FEWidth = SafeCDbl(value)
            If mediatype = Video Or mediatype = "Live" Then FEWidth = ConfigInfo.MediaWidth
        Case "Fish_OriginalHeight="
            FEHeight = SafeCDbl(value)
            If mediatype = Video Or mediatype = "Live" Then FEHeight = ConfigInfo.MediaHeight
        Case "Fish_Ratio=":                 FEScale = SafeCDbl(value)
      End Select
...
'Select Case FECRev
    ...
    If FECRev = "2.0" Or FECRev = "2.1" Then
        ...
        If mediatype <> Video And mediatype <> "Live" Then
...
Error_handler:
    Select Case Err
        Case Else: MsgBox Err & "-FF12:" & Error$, vbExclamation
    End Select
End Sub
...
Error_handler:
    Select Case Err
        Case Else: MsgBox Err & "-FF13:" & Error$, vbExclamation
    End Select
End Function
...
Error_handler:
    Select Case Err
        Case Else: MsgBox Err & "-FF14:" & Error$, vbExclamation
    End Select
End Sub
